 ...
 *
 * Deterministic, greedy-then-shrink:
 *   1. Header (runId, aggregate status, failed-step count, failed-step list).
 *   2. For each failing job (ordered by workflow id, then job id):
 *        - job header: `[<runnerId>] failed at "<failedStep>" (<kind>)`
 *        - up to `maxFailingTests` failing tests
 *            - each rendered: `- <name>` + `at <location>` + indented errorLines
 *        - if no parseable failing tests, fall back to the tail excerpt.
 *   3. Token-budget check via `estimateTokens`. If over:
 *        a. cap failing tests per job at 3, then 1;
 *        b. drop whole jobs from the tail, one at a time;
 *        c. last resort: header only.
 *
 * Output is ASCII + unicode, no ANSI. Newline-joined.
 */
...
/**
 * Project a CIRunSummary into a CI domain event. Green -> PatchReady
 * (no excerpt). Red -> CIRed with a pre-rendered reviewer excerpt.
 */
...
/**
 * Character-based token estimate. ~3.5 chars/token is a conservative rough
 * approximation for English + code. Deliberately overestimates so callers
 * stay under their real token budget.
 */
