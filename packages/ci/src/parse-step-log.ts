/**
 * Step-log extractors: TAP-13 (node:test / tap / ava), ESLint stylish, and
 * a tail fallback used when no format-specific parser recognised the output.
 *
 * These produce a `ParsedStepLog` that feeds `parseRunState` and, through it,
 * the reviewer excerpt. Deliberately zero network / TAP-parser deps — the
 * contract is owned entirely by this package.
 */

import { stripAnsi } from "./ansi.ts";
import type { FailingTest, FailureKind, ParsedStepLog } from "./types.ts";

/**
 * Classify a step by its name. Heuristic: agent-ci reports step names verbatim
 * from workflow YAML, so we match common conventions. Reviewers always see the
 * raw step name — this only drives layout (test-failure rendering vs lint).
 */
...
/**
 * Parse TAP 13 output for failing tests. Recognises:
 *
 *   not ok N - <name>
 *     ---
 *     <yaml block>
 *     ...
 *
 * and the Node test-runner's nested indentation. Payload lines are captured
 * verbatim (minus the leading indent); `location`, `expected`, and `actual`
 * are lifted from the YAML when present.
 */
export function parseTapFailures(
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Look ahead a few lines for a YAML block `---` ... `...`.
    let j = i + 1;
    ...
    if (yamlStart !== null) {
      ...
      // Determine YAML body indent from the first non-blank line. Node's test
      // runner nests at 6 spaces; agent-ci step logs often flatten to 0. Be
      // tolerant.
      ...
      const dedented = block.map((l) =>
        l.length >= stripIndent ? l.slice(stripIndent) : l.trimStart(),
      );
      ...
      // If no error block, fall back to the first N non-YAML-key lines.
    ...
    const record: FailingTest = {
      name: name || `unnamed test ${failures.length + 1}`,
      location,
      errorLines,
    };
    if (expected !== undefined) record.expected = expected;
    if (actual !== undefined) record.actual = actual;
    failures.push(record);
  }

  // Drop Node test-runner suite-level roll-ups ("N subtest failed"); the
  // real subtest failures are already captured as siblings.
  return failures.filter((f) => {
    const first = (f.errorLines[0] ?? "").trim();
    return !/^['"]?\d+ subtest(s)? failed['"]?$/.test(first);
  });
}
...
/**
 * Parse ESLint default (stylish) output for errors. Expected shape:
 *
 *   <file path>
 *     L:C  error    message              rule/name
 *
 * Forgiving about leading whitespace and repeated spaces. Warnings are
 * ignored — the gate cares about errors.
 */
...
/**
 * Last-resort extractor: capture the last N non-blank lines of a step log,
 * ANSI stripped. Used when no format-specific parser recognised the output.
 */
...
export function parseStepLog(
  ...
  const extractorOpts: { maxErrorLinesPerTest?: number } =
    opts.maxErrorLinesPerTest !== undefined
      ? { maxErrorLinesPerTest: opts.maxErrorLinesPerTest }
      : {};

  let failures: FailingTest[] = [];
  if (kind === "test") {
    failures = parseTapFailures(raw, extractorOpts);
  } else if (kind === "lint") {
    failures = parseEslintFailures(raw, extractorOpts);
  }
